...
def main(tracker, thread_coco, thread_image):
    ...
    while True:
        ...
        # No need to convert as already in right format
        image_np = thread_image.image_data.image_np.copy()
        output_data = thread_coco.output_data
        output_data = tracker.run(output_data, image_np)
        image_np = helper.drawDetectedBBs(
            image_np.copy(), output_data, score_thresh=0.1
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--input', default='cam', help='"cam" for Webcam or video file path'
    )
